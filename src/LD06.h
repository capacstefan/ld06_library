...
class LD06{                                                                                        // LD06 will be the class of the lidar
  public:
    ...
    std::vector<std::vector<uint8_t>> getMap(int RANGE, int MAP_SIZE, float START_ANGLE, float END_ANGLE); // Building a grid using the points read between certain degrees
  	...
  	std::vector<IndexedPoint> getIndexedPointsLite(int RANGE, int MAP_SIZE);					   // Lite Version of main method
    std::vector<IndexedPoint> getIndexedPoints(int RANGE, int MAP_SIZE, float START_ANGLE, float END_ANGLE); // Returning only the points in specified area
	...
	std::vector<RawPoint> getRawPointsLite(int RANGE);											   // Lite Version of main method
    std::vector<RawPoint> getRawPoints(int RANGE, float START_ANGLE, float END_ANGLE);			   // Returning only the points in specified area
